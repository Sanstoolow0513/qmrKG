...
每个抽取出的实体除名称和类型外，还要求 LLM 生成简短描述（description 字段），为例，实体"TCP"的描述为"传输控制协议"。引入描述字段有两重目的：为下游知识融合阶段提供语义信息；为知识图谱的交互式浏览提供可读的节点信息。
...
实体规范化完成后，系统对三元组进行合并与去重。合并策略：以规范化后的（$head$, $relation$, $tail$）三元组作为键分组，对同一键下的三元组累加 $frequency$ 频次，并收集所有唯一的 $evidences$ 证据文本。合并后过滤自环三元组及头尾实体不在有效集合中的三元组。保留频次信息可使高频实体在知识图谱可视化中更为突出，并作为实体重要性的量化指标辅助知识检索。

\subsection{图存储与可视化设计概要}
...
查询策略采用\textbf{诱导子图采样}：按频次取前 $N$ 个高频节点，查询这些节点之间的所有关系构成诱导子图。选择该策略的原因是：完整知识图谱的直接渲染会导致视图混乱和性能下降，诱导子图采样在保留核心知识关联的同时，将可视化规模控制在合理范围。节点和关系数量上限通过环境变量配置。

\subsubsection{前端可视化架构设计}

前端使用 Next.js 16 框架，基于 App Router 架构搭建。可视化核心使用 react-force-graph-2d 库，以 d3-force 引擎为底层进行力导向布局计算。设计要点包括：对数节点映射（半径 2.5--11px）避免高频节点过度放大；多边曲线交替偏移使每条边在视觉上可区分；缩放感知标签仅在缩放比例超过 1.4 倍时显示，避免视觉噪声；提供实体类型和关系类型的复选框过滤功能。前端组件的代码结构与主题配置见第四章。
