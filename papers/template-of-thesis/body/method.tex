...
课程教材层级结构深、术语密集、中英文混用，从中抽取实体和关系比通用场景更容易出错。本章从需求分析入手，明确系统的功能边界与约束条件；随后给出总体架构设计，再按文档预处理、文本分块、实体识别、关系抽取、知识融合、图存储与可视化六个设计模块，逐一说明设计决策和取舍理由。
...
\textbf{图存储与可视化能力。} 融合后的知识图谱应导入 Neo4j 图数据库持久化存储，并通过 Web 前端提供交互式浏览功能，支持节点过滤、关系导航和缩放查看。

\subsubsection{界面（人机交互）需求}

系统的目标用户为课程教师和学生。主要交互方式分为命令行工具和 Web 可视化界面两类。
...
\textbf{Web 可视化界面。} 前端提供力导向图布局的知识图谱浏览，支持：按实体类型和关系类型多维度过滤；缩放感知的节点标签显示；多边关系曲线的视觉区分；点击节点查看实体描述和关联证据。
...
\textbf{图数据库 $\to$ 前端}：RESTful API（\texttt{/api/graph}），返回节点数组和边数组的 JSON 格式。
...
算法\ref{al:3-1}统一表达了标题树构建、递归判定和段落级兜底切分过程。几个关键设计决策包括：（1）标题层级信息保留在每个输出块的元数据（$titles$ 字段），供下游抽取阶段做上下文定位；（2）清洗步骤在分块之前执行，避免 OCR 伪影（如页码、页眉）污染文本块内容；（3）段落级累积切分作为兜底手段，确保即使没有标题结构的长文本也能正确分块。
